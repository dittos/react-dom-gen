...
// var inputValueTracking = require('react-dom/lib/inputValueTracking');
...
ReactDOMServerComponent.Mixin = {
  ...
  mountComponent: function(
    ...
    if (transaction.useCreateElement) {
      ...
      transaction.serverBuffer.write(tagOpen);
      transaction.serverBuffer.enqueueNextWriteHeader('>');
      var pos = transaction.serverBuffer.position;
      this._createContentMarkup(transaction, props, context);
      var isEmpty = pos === transaction.serverBuffer.position;
      if (isEmpty && omittedCloseTags[this._tag]) {
        transaction.serverBuffer.resetNextWriteHeader();
        transaction.serverBuffer.write('/>');
      } else {
        transaction.serverBuffer.write('</' + type + '>');
      }
    }
  ...
  _createContentMarkup: function(transaction, props, context) {
    ...
    if (newlineEatingTags[this._tag] && ret.charAt(0) === '\n') {
      ...
      if (write)transaction.serverBuffer.write('\n'+ret);
      return '\n' + ret;
    } else {
      if (ret&&write)transaction.serverBuffer.write(ret);
      return ret;
    }
  },
